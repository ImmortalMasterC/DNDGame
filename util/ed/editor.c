 ...
 *		Modified by Roy Wilson, 1997, 1998  --  styx@styx.darbonne.com
 */
...

{
	...
	while(1) {
		clearscreen();
		posit(8,30);  printf("Darbonne Editor");
		...
		posit(14,30); printf("5. Plural Form");
...

{
	...
	while(1) {
		...
		case 5:			/* shots/dmg/armor/wear/magic */
			clearscreen();
			posit(2,1); printf(" Shots: %-5d/%-5d", obj_ptr->shotscur, 
				obj_ptr->shotsmax);
...

{
	...
	while(1) {
		...
		posit(6,30);  printf(" 3. Edit level/class/align/race/stats");
		...
		case 3:			/* level/class/align/stats */
			...
			posit(2,60); printf("Race: %-3d", crt_ptr->race);
			...
			crt_ptr->alignment = j;
			posit(2,66); getnums(&j, (long)crt_ptr->race, -127L, 127L);
			crt_ptr->race = j;
		...
		case 4:			/* HP/MP/Armor/Thaco */
			clearscreen();
			posit(2,1); printf("   HP: %-5d/%-5d", crt_ptr->hpcur, 
				crt_ptr->hpmax);
			posit(2,30); printf("   MP: %-5d/%-5d", crt_ptr->mpcur, 
				crt_ptr->mpmax);
			...
			crt_ptr->numwander = j;
			break;
		case 5: 		/* exp/gold/bank/dmg/quest */
			clearscreen();
		...
		case 6:		   	/* spells */
			...
			posit(6,40); printf("1");
			posit(6,50); printf("1");
			posit(3,1); printf("123456789012345678901234567890"
							   "1234567890123456789012345678901234567890");
			posit(8,1); printf("123456789012345678901234567890"
		...
		case 8:
			...
			posit(3,3); getnums(&j, (long)crt_ptr->proficiency[0], 
										0L, 1000000L);
			crt_ptr->proficiency[0] = j;
			posit(3,29); getnums(&j, (long)crt_ptr->proficiency[1], 
										0L, 1000000L);
			crt_ptr->proficiency[1] = j;
			posit(3,56); getnums(&j, (long)crt_ptr->proficiency[2], 
										0L, 1000000L);
			crt_ptr->proficiency[2] = j;
			posit(4,3); getnums(&j, (long)crt_ptr->proficiency[3], 
										0L, 1000000L);
			crt_ptr->proficiency[3] = j;
			posit(4,29); getnums(&j, (long)crt_ptr->proficiency[4], 
										0L, 1000000L);
	...
	while(1) {
		...
		case 7:			/* Perm monsters */
			...
			while(1) {
				...
				posit(x+4, 10); getnums(&j, 
					(long)rom_ptr->perm_mon[x-1].interval, 0L, 10000000L);
				rom_ptr->perm_mon[x-1].interval = j;
				posit(x+4, 20); getnums(&j, 
					(long)rom_ptr->perm_mon[x-1].ltime, 0L, 10000000L);
				rom_ptr->perm_mon[x-1].ltime = j;
				posit(x+4, 30); getnums(&j, 
					(long)rom_ptr->perm_mon[x-1].misc, 0L, 32000L);
		...
		case 8:			/* Perm objects */
			...
			while(1) {
				...
				if(!x) break;
				posit(x+4, 10); getnums(&j, 
					(long)rom_ptr->perm_obj[x-1].interval, 0L, 10000000L);
				rom_ptr->perm_obj[x-1].interval = j;
				posit(x+4, 20); getnums(&j, 
					(long)rom_ptr->perm_obj[x-1].ltime, 0L, 10000000L);
				rom_ptr->perm_obj[x-1].ltime = j;
				posit(x+4, 30); getnums(&j, 
					(long)rom_ptr->perm_obj[x-1].misc, 0L, 32000L);
				rom_ptr->perm_obj[x-1].misc = j;
			}
			break;
		case 9:			/* Statistics */
			clearscreen();
			posit(2,1); printf("     Visits: %lu", 
				(unsigned long)rom_ptr->beenhere);
			posit(3,1); printf("Established: %lu", 
				(unsigned long)rom_ptr->established);
...

{
	...
	while(1) {
		...
		case 1:			/* Add exit */
			...
			posit(11,40); printf("4");
			posit(11,50); printf("5");
			posit(11,60); printf("6");
			posit(12,1); printf("123456789012345678901234567890"
							    "1234567890123456789012345678901234");
		...
		for(j=0; j<64; j++)
			...
			xt->ext->key = j;
			posit(7,16); getnums(&j, (long)xt->ext->ltime.interval, 
								0L, 10000000L);
...
/************************************************************************/
/*  Some words require special handling to create the plural form,      */
/*  such as elf and elves.  This section allows the user to add both    */
/*  the singular form and its plural form to a file where it can be     */
/*  found in a look-up table when more than one monster or object is    */
/*  in a room.    -Styx-						*/

void plural()
{
char	sform[80];	/* singular form */
char	pform[80];	/* plural form   */
char	pfile[80];
FILE	*plural;  

	clearscreen();
	posit(8,20);  printf("The plurals of some words are irregular");
	posit(9,20);  printf("and require special attention.");
	posit(10,20);  printf("Example:  mouse and mice");
	posit(12,20);  printf("Singular form:   ");
	gets(sform);
	if(strlen(sform) > 0) {
		posit(13,20);  printf("  Plural form:   ");
		gets(pform);
	}

/*  WRITE BOTH SINGULAR AND PLURAL FORMS TO THE FILE PLURAL_LIST. */

	if(strlen(sform) > 0 && strlen(pform) > 0) {
	    strcpy(pfile, OBJPATH);
	    strcat(pfile, "/plural_list");
	    plural = fopen(pfile, "a");
	    if(fopen(pfile, "a") == NULL)
	 		printf("\nError.  plural_list file could not be opened.\n");
	    else {
			fprintf(plural, "%s\n%s\n", sform, pform);   
			fclose(plural);
	   }
