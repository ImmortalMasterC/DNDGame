...
*  Version 1.0:  (C) 1996 Brooke Paul & Eric Krichbaum
...
char    *argv[];
{
	...
	for(rnum=Rlow; rnum<Rhi; rnum++) {
		...
		while(cp) {
			...
			if(!found) {
				if(verbose) {
					sprintf(str, "Room %d has a bad perm creature: %s.", 
						rnum, cp->crt);
					if(!spinner)
						write(fd, str, strlen(str));
					else
...
/* add monster to DB */
				if(!n && writeok) {
					...
					close(ff);
					sprintf(str, "### Creature %d updated for room %d.", 
						dbm, rnum);
					if(!spinner)
						write(fd, str, strlen(str)); 
					else
...
/* properly perm monster */
				...
				if(l>9) {
					sprintf(str, 
						"### Room %d is already full of perm creatures.", 
							rom->rom_num);
					if(!spinner)
						write(fd, str, strlen(str));  
					else
		...
		while(rom->perm_mon[l].misc && l<10) {
			if(rom->perm_mon[l].interval < 5) {
				sprintf(str, "### Interval fixed on monster %d in room %d.", 
					rom->perm_mon[l].misc, rnum);
				if(!spinner)
					write(fd, str, strlen(str));	
				else
					message(str);
		...
		while(op) {
			...
			if(!found) {
				if(verbose) {
					sprintf(str, "Room %d has a bad perm object: %s.", 
						rnum, op->obj);
					if(!spinner)
						write(fd, str, strlen(str)); 
					else
				...
				if(!n && writeok) {
        			...
        			close(ff);
					sprintf(str, "### Object %d updated for room %d.", 
						dbo, rnum);
					if(!spinner)
						write(fd, str, strlen(str)); 
					else
		...
		/* properly perm object */
				...
				if(l>9) {
					sprintf(str, "### Room %d is already full of perm objects.",
						rom->rom_num);
					if(!spinner)
							write(fd, str, strlen(str)); 
					else
		...
		while(rom->perm_obj[l].misc && l<10) {
			if(rom->perm_obj[l].interval < 5) {
				sprintf(str, "### Interval fixed on object %d in room %d.", 
					rom->perm_obj[l].misc, rnum);
				if(!spinner)
					write(fd, str, strlen(str));
				else
					message(str);
