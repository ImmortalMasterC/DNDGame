...

{
	...
	ANSI(fd, BOLD);
	ANSI(fd, YELLOW);
	print(fd, "%9s %-15.15s", "Lev/Class", " Player");

	if(fulluser)
		print(fd, "%-47.47s", "Email address");
	else
		print(fd, "%-22.22s %-25.25s", "Room & Name", "Last command");
	...
	for(i = 0; i < Tablesize; i++) {
		...
		print(fd, "%-6.7s ", class_str[Ply[i].ply->class]);
		ANSI(fd, BOLD);
		ANSI(fd, WHITE);
			print(fd, "%s%-12.12s ",
