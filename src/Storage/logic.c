/*
 *  Logic Creature Script added by Charles Marchant
 * 
 */
	...
	while(!feof(fp)) {
		...
		while(*ptr) {	     
			switch(*ptr) {
				...
				case '!': /* if last command not successful goto cmd */
					ptr++;
					act->if_cmd = atoi(ptr);
					for(; *ptr && *ptr != ':'; ptr++);
					ptr++;
				...
				case '=': /* if last command was succesful goto cmd */
					ptr++;
					act->if_cmd = atoi(ptr);
					for(;*ptr && *ptr != ':'; ptr++);
					ptr++;
				...
				default:
					...
					logn("Errors", "UNKNOWN LOGIC COMMAND [%s] in %s\n", 
						cmdstr, filename);
					act->do_act = 0;
					act->goto_cmd = 1;
					for(; *ptr; ptr++);
