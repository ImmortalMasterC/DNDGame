...
/*			player_inv				          */
/**************************************************************************/
/* Returns the number of objects a player has in his inventory. 	  */

int player_inv(ply_ptr)
creature 	*ply_ptr;
...

{
	...
	while(op) {
		if(!perm_only || (perm_only && (F_ISSET(op->obj, OPERMT)))) {
			if(F_ISSET(op->obj, OCONTN) && (F_ISSET(op->obj, OPERMT)))
				pcontain =  ALLITEMS;
			else
				pcontain = perm_only;
...
/*			free_rom				         */
/*************************************************************************/
/* This function releases a room and its contents from memory.  The      */
