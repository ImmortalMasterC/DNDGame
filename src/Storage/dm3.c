...

{
	...
	else if(!strcmp(name, "l")) strcpy(name, "left");
	else if(!strcmp(name, "r")) strcpy(name, "right");
	else if(!strcmp(name, "po")) strcpy(name, "port");
	else if(!strcmp(name, "st")) strcpy(name, "starboard");
}
...
/*			dm_finger						   */
/***********************************************************************************/
/* This command allows a DM to finger the person named.  The DM can also request   */
/* the site he is fingering using the @ in front of the address.   		   */
...

{
	...
	if(cmnd->str[1][0] == '+' || (cmnd->str[1][0] == '-')) {
		...
		for(i = 0; i < Tablesize; i++) {
                        ...
                        print(fd, "%13.12s ", Ply[i].ply->name);
