...

{
   ...
   if(cmnd->num == 2) {
        ...
        if(how == CAST || how == SCROLL || how == WAND)  {
		    ...
		    ANSI(fd, RED);   	
            print(fd, 
				"You have gone blind in both eyes and can no longer see!\n");
		    ANSI(fd, BOLD);
		    ANSI(fd, WHITE);   	
            broadcast_rom(fd, ply_ptr->rom_num, 
			...
			CLS(fd);
            print(fd, 
				"Everything goes dark.  You have gone blind in both eyes!\n");
    	}
    }
		...
		crt_ptr->class == DRUID) {
            ...
            broadcast_rom2(fd, crt_ptr->fd, ply_ptr->rom_num,
            	"%M reflects the blindness spell back to %m.", 
					crt_ptr, ply_ptr);
            print(crt_ptr->fd, 
				"You reflect the blindness spell back against %M.\n", ply_ptr);
        }
