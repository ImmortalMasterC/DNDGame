...
/*      				        drink 		 			               	*/
/************************************************************************/
/* This function allows a player to drink a potion, thereby casting    	*/
...

{
	...
	if(F_ISSET(ply_ptr->parent_rom, RNOPOT)) {
        print(fd, "%I evaporates before you drink it.\n", obj_ptr);
    	obj_ptr->shotscur = 0; 
        return(0);
	}
...

{
    ...
    /* Cast on monster or player */
        ...
        if(how == POTION) {
            print(fd, "How do we force anyone to drink it?\n");
            return(0);
        }
