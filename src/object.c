...
/* contains the number of the string being searched for.				*/
...

{
	...
	while(op) {
		if(EQUAL(op->obj, str) && ((F_ISSET(ply_ptr, PDINVI) || 
...
/* is a pointer to the first object tag in the list of items that is	*/
/* being listed.														*/
	...
	while(op && strlen(str) < 2048) {
		if((F_ISSET(ply_ptr, PDINVI) ? 1:!F_ISSET(op->obj, OINVIS)) &&
			!F_ISSET(op->obj, OHIDDN) && !F_ISSET(op->obj, OSCENE)) {
				...
				if(strlen(str2) + strlen(str) < 2048) {
					strcat(str, str2);
					strcat(str, ", ");
					n++;
				}
