		...
		while(op) {
			if(!perm_only || (perm_only && (F_ISSET(op->obj, OPERMT) || 
				F_ISSET(op->obj, OPERM2)))) {
					if((n = write_obj_to_mem(buf, op->obj, perm_only)) < 0)
						error = 1;
					else
						buf += n;
					cnt2++;
...
/*								write_crt_to_mem						*/
/************************************************************************/
/*   Save a creature to memory.  This function also saves all the items */
/*   the creature is holding.  If perm_only != 0 then only those items  */
/*   which the creature is carrying that are permanent will be saved.	*/
